...
> 입력 근거: 마스터 플랜 §10 S2 + 본 sprint line-level 실측. 산출물: `docs/01-plan/features/cross-platform-mac-windows.plan.md` · `docs/02-design/features/cross-platform-mac-windows.design.md` · `docs/04-report/features/cross-platform-mac-windows.report.md`. **실측 교정**: 마스터 플랜 worst-case 추정(raw concat 14 / shell 분기 확대 / 21 hook 위험)은 과대 — bkit는 이미 ~90% cross-platform-safe(`path.join` 349, raw `__dirname+'/'` concat 0, exec는 git/gh/node/npx만, POSIX coreutils exec 0). 진짜 위험은 **CRLF 미처리 frontmatter/markdown 파서** 단일 카테고리.

- **ENH-331 — CRLF/LF 처리 (P0, 하드 브레이크 수정)**: Windows CRLF(`---\r\n`) 파일에서 frontmatter fence 정규식 `/^---\n.../` 가 **매칭 완전 실패** → skill/agent/output-style 로딩이 깨지던 결함 수정. (a) fence 정규식 6곳 `\r?\n` 적용: `lib/util/markdown-parse.js:49`, `lib/qa/utils/pattern-matcher.js:186`, **`hooks/startup/context-init.js:146`(런타임 hook)**, `scripts/validate-plugin.js:83`, `scripts/audit-output-styles.js:21/45`. (b) lib/ 파일내용 `split('\n')` → `split(/\r?\n/)` **34 사이트 / 19 파일**(skill-orchestrator·import-resolver·pattern-matcher·workflow-parser·discovery/explorer·audit·cc-regression 등). *이미 `\r?\n` 였던 skill-orchestrator:60·frontmatter.js:57/109·import-resolver:176은 검증완료(변경 불요).*
- **ENH-329 — Path separator 정합성**: `scripts/check-deadcode.js` 모듈경로 비교에 backslash 정규화(`.replace(/\\/g,'/')`) 추가(win32 glob/fs 결과 대비). `lib/qa/utils/file-resolver.js:76` glob 패턴 split은 forward-slash 규약(fs 경로 아님)임을 주석 명시. `lib/ui/impact-view.js` 는 이미 정규화됨(검증완료).
- **ENH-330 — Shell 분기 검증 (YAGNI)**: exec 18 파일 전수 — `git`/`gh`/`node`/`npx` 만 사용, POSIX coreutils(find/wc/grep/cat) exec **0** → **bash↔pwsh 분기 추가 불요** 확정. 기존 `process.platform` 2곳(defense-coordinator·enh-254-fork-precondition)으로 충분. "분기 불요" 근거를 향후 회귀 기준선으로 design에 명문화.
- **ENH-332 — Hook Windows 발화 검증**: hooks.json 25 command 전부 `node "${CLAUDE_PLUGIN_ROOT}/<path>.js"` 형식(node가 win32에서 forward-slash 허용 → 안전), shebang `#!/usr/bin/env node` 61파일은 `node` 직접 호출로 inert(win32 무관). 8 hook .js 중 context-init fence만 수정 필요(ENH-331 포함).
