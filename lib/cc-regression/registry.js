...
const CC_REGRESSIONS = [
  ...
  // --- v2.1.22 S6 (ENH-366): CC strict Stop hook output validation ---
  // Reference: docs/03-analysis/features/cc-stop-hook-schema-compliance.analysis.md
  // Same class as R3-321 (plugin manifest strict reject): CC now strict-validates
  // hook output JSON. bkit Stop emitters had been emitting decision:'allow'
  // (Stop decision is approve|block), Stop-unsupported hookSpecificOutput, and
  // non-schema root fields (skillResult/autoTrigger/iterationResult/analysisResult)
  // → "(root): Invalid input". Confirmed live on CC v2.1.159 (introduced version
  // unknown — pin in future reconcile). bkit-controlled surface → fully resolved
  // by S6 (5 emitters → io.outputStopSurface/outputStopAllow). Permanent guard:
  // test/contract/stop-hook-output-schema.test.js.
  {
    id: 'MON-CC-NEW-STOP-SCHEMA-STRICT',
    issue: 'https://github.com/anthropics/claude-code/issues/26555',
    severity: 'HIGH',
    since: '2.1.159',        // confirmed live (introduced version unknown — reconcile pin TODO)
    expectedFix: null,        // CC behavior is intentional strictness, not a bug to fix
    affectedFiles: [
      'scripts/sprint-skill-stop.js',
    ...
    resolvedAt: '2026-06-01', // bkit-controlled — resolved by S6 (decision:'block'+reason / {} clean stop)
    notes: 'P0 활성 breakage (사용자 /sprint list 중 confirmed). root=cc-payload.port.js decision 타입 오류(allow|deny|ask|defer) 전파 → 5 Stop emitter decision:\'allow\' + Stop hookSpecificOutput + skillResult/autoTrigger/iterationResult/analysisResult root field. CC strict Stop validator reject. bkit S6 ENH-361~366 fix: io.outputStopSurface(decision:\'block\'+reason)/outputStopAllow({}) 공유 헬퍼 단일화, port typedef 정정, contract test 가드. 런타임 QA 5/5 emitter 스키마 통과.',
  },
];
